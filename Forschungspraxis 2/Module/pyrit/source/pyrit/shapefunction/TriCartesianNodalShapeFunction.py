...
def calc_divgrad_constant_scalar(elements: np.ndarray, i: np.ndarray, j: np.ndarray, v: np.ndarray,
                                 value: Union[int, float], b: np.ndarray, c: np.ndarray, length: float,
...
def calc_divgrad_constant_tensor(elements: np.ndarray, i: np.ndarray, j: np.ndarray, v: np.ndarray,
                                 ...
                                 length: float, determinant_b: np.ndarray,
                                 elem2node) -> NoReturn:
    """
...
def calc_divgrad_scalar_per_elem(elements: np.ndarray, i: np.ndarray, j: np.ndarray, v: np.ndarray,
                                 value: np.ndarray, b: np.ndarray, c: np.ndarray, length: float,
                                 determinant_b: np.ndarray,
                                 elem2node):
...
def calc_mass_function_scalar(elements: np.ndarray, i: np.ndarray, j: np.ndarray, v: np.ndarray,
                              ...
                              evaluation_points: np.ndarray, evaluations: np.ndarray, length: float,
                              determinant_b: np.ndarray, elem2node: np.ndarray):
...
def calc_load_elems_constant(elements: np.ndarray, i: np.ndarray, v: np.ndarray,
                             value: Union[int, float], determinant_b: np.ndarray,
                             elem2node) -> NoReturn:
    ...
    for k in prange(num_elements):
        element = elements[k]
        vec_elem = determinant_b[element] * value / 6
        idx_node = elem2node[element, :]
...
def calc_load_elems_vector(elements: np.ndarray, i: np.ndarray, v: np.ndarray,
                           value: Union[int, float], length: float, determinant_b: np.ndarray,
                           elem2node) -> NoReturn:
    """Calculates elements for the load vector with a constant load.
    ...
    for k in prange(num_elements):
        element = elements[k]
        vec_elem = determinant_b[element] * value[k] / 6
        idx_node = elem2node[element, :]
...
def calc_curl(val2edge: np.ndarray, node: np.ndarray, elem2node: np.ndarray, num_elem: int, b: np.ndarray,
    ...
    for k in prange(num_elem):
        nodes = elem2node[k]
        radius_mid = 1 / 3 * np.sum(node[nodes, 0])  # TODO adjust to cartesian
...
@njit_cache(parallel=True)
def calc_neumann_term_constant(edges: np.ndarray, i: np.ndarray, v: np.ndarray, value: Union[int, float], length: float,
    ...
    for k in prange(num_edges):
        ...
        i_local[k] = idx_node
        v_local[k] = value * 0.5 * length_edge * length
...
@njit_cache(parallel=True)
def calc_neumann_term_array(edges: np.ndarray, i: np.ndarray, v: np.ndarray, value: np.ndarray, length: float,
                            node: np.ndarray, edge2node: np.ndarray):
    """Calculates elements for the neumann term vector with a constant value.
    ...
    for k in prange(num_edges):
        ...
        indices_v[k] = np.arange(2 * edge, 2 * edge + 2, 1)
        i_local[k] = idx_node
        v_local[k] = value[k] * 0.5 * length_edge * length
...
class TriCartesianNodalShapeFunction(NodalShapeFunction):  # TODO NEXT STEP anisotrop
    """Class representing shape functions in axisymmetric geometries with a triangular mesh in the rz-plane."""

    def __init__(self, mesh: 'AxiMesh', length: float = 1):
        """Constructor.
    ...
    def _calc_matrix_constant_scalar(self, matrix_type: str, indices: np.ndarray, i: np.ndarray, j: np.ndarray,
        ...
        if matrix_type == "divgrad":
            calc_divgrad_constant_scalar(indices, i, j, v, value, self.b,
                                         self.c, self.length, self.__determinant_b, self.mesh.elem2node)
        elif matrix_type == "mass":
            calc_mass_constant_scalar(indices, i, j, v, value, self.length, self.__determinant_b, self.mesh.elem2node)
        else:
    ...
    def _calc_matrix_constant_tensor(self, matrix_type: str, indices: np.ndarray, i: np.ndarray, j: np.ndarray,
        ...
        if matrix_type == "divgrad":
            calc_divgrad_constant_tensor(indices, i, j, v, value.astype(float), self.b,
                                         self.c, self.length, self.__determinant_b,
                                         self.mesh.elem2node)
        elif matrix_type == "mass":
    ...
    def _calc_matrix_scalar_per_elem(self, matrix_type: str, indices: np.ndarray, i: np.ndarray, j: np.ndarray,
        ...
        if matrix_type == "divgrad":
            calc_divgrad_scalar_per_elem(indices, i, j, v, evaluations,
                                         self.b, self.c, self.length, self.__determinant_b, self.mesh.elem2node)
        elif matrix_type == "mass":
            calc_mass_scalar_per_elem(indices, i, j, v, evaluations, self.length, self.__determinant_b,
                                      self.mesh.elem2node)
        else:
    ...
    def _calc_matrix_function_scalar(self, matrix_type: str, indices: np.ndarray, i: np.ndarray, j: np.ndarray,
        ...
        if matrix_type == "divgrad":
            calc_divgrad_function_scalar(indices, i, j, v, weights, evaluation_points,
                                         evaluations, self.b, self.c, self.length, self.__determinant_b,
                                         self.mesh.elem2node)
        elif matrix_type == "mass":
    ...
    def _calc_matrix_function_tensor(self, matrix_type: str, indices: np.ndarray, i: np.ndarray, j: np.ndarray,
        ...
        if matrix_type == "divgrad":
            calc_divgrad_function_tensor(indices, i, j, v, weights, evaluation_points, evaluations,
                                         self.b, self.c, self.length, self.__determinant_b,
                                         self.mesh.elem2node)
        elif matrix_type == "mass":
            raise NotImplementedError('Mass matrix does not support tensors.')
        else:
        ...
        if case == "tuple":  # material is a tuple
            ...
            for key in regions.get_keys():
                ...
                if regi.dim == 0:
                    ...
                    if exci.is_constant:
                        calc_neumann_term_constant(indices, i_face, v_face, exci.value, self.length, self.mesh.node,
                                                   self.mesh.edge2node)
                    elif not exci.is_homogeneous:
                        ...
                        value = exci.value()
                        if len(value) == 1:
                            calc_neumann_term_constant(indices, i_face, v_face, value, self.length, self.mesh.node,
                                                       self.mesh.edge2node)
                        elif len(value) == len(indices):
                            calc_neumann_term_array(indices, i_face, v_face, value, self.length, self.mesh.node,
                                                    self.mesh.edge2node)
                        else:
                            raise NotImplementedError
                    ...
                    if exci.is_constant:
                        ...
                        if not exci.is_homogeneous:
                            ...
                            calc_load_elems_function(indices, i_elem, v_elem, weights, local_coordinates,
                                                     evaluation_points,
                                                     evaluations, self.length, self.__determinant_b,
                                                     self.mesh.elem2node)
            ...
            calc_load_elems_constant(np.arange(self.mesh.num_elem), i_elem, v_elem, load, self.__determinant_b,
                                     self.mesh.elem2node)
            ...
            calc_load_elems_function(np.arange(self.mesh.num_elem), i_elem, v_elem, weights, local_coordinates,
                                     evaluation_points, evaluations, self.length, self.__determinant_b,
                                     self.mesh.elem2node)
        else:
    ...
    def neumann_term(self, *args: Union[Tuple['Regions', 'BdryCond'],
        ...
        if flag_regions:  # We have regions and boundary_conditions
            ...
            for key in boundary_conditions.get_ids():
                ...
                if not bc.is_homogeneous:
                    ...
                    if isinstance(bc.value, (float, int)):
                        calc_neumann_term_constant(indices_edges, i, v, bc.value, self.length, self.mesh.node,
                                                   self.mesh.edge2node)
                    elif isinstance(bc.value, ndarray):
                        calc_neumann_term_array(indices_edges, i, v, bc.value, self.length, self.mesh.node,
                                                self.mesh.edge2node)
                    else:
            ...
            if flag_value == 'callable':
                ...
                calc_neumann_term_array(indices, i, v, value, self.length, self.mesh.node, self.mesh.edge2node)
            elif flag_value == 'value':
                calc_neumann_term_constant(indices, i, v, value, self.length, self.mesh.node, self.mesh.edge2node)
        columns = np.zeros(len(i), dtype=int)
